...
##prediction model
def test_model(inputfile, outputfile, species, region, seqtype):

    sORF_seq = read_fasta('../input_files/' + inputfile, seqtype)
    protein_seq = get_protein(sORF_seq)
    fea = extracted_features(sORF_seq, protein_seq, species, region)
...
parser.add_argument('-s', dest='speciesname', type=str, required=False,
                    help='Please enter the specific species, currently we accept three options: H.sapiens, M.musculus, and D.melanogaster')
parser.add_argument('-r', dest='regiontype', type=str, required=False,
                    help='Please enter the region type to choose the model, two options: CDS and non-CDS')
parser.add_argument('-t', dest='seqtype', type=str, required=True, choices=['sORF', 'other'], help='Query sequence is sORF or other')
args = parser.parse_args()

for file in ([args.inputfile, args.outputfile, args.speciesname, args.regiontype, args.seqtype]):
 if not (file):
    print(sys.stderr, "\nError: Lack of input file!\n")
    parser.print_help()
    sys.exit(0)

test_model(args.inputfile, args.outputfile, args.speciesname, args.regiontype, args.seqtype)
